...
- **'Lower layer Super Output Areas (December 2021) Boundaries EW BSC (V4)'** from the **Office for National Statistics Open Geography Portal**. This contains digital vector boundaries for Lower layer Super Output Areas in England and Wales, as at 21 March 2021.
...
#load data
#1: unzip the data - unpack it from the format it was downloaded to my computer in. we made it a special file within the 'data' file of the Project
unzip("data/postcode_boundary_data/UK-postcode-boundaries-Jan-2015.zip")
unzip("data/LSOAs_2021/Lower_layer_Super_Output_Areas_December_2021_Boundaries_EW_BSC_V4_-5236167991066794441)"


#now we know the file name: load the data into R, and call it 'cons_sf'
postcodes_sf<-read_sf("data/postcode_boundary_data/Distribution/Sectors.shp")
lsoas_sf<-read_sf("data/LSOAs_2021/LSOA_2021_EW_BSC_V4.shp")


#have a look at it 
postcodes_sf
head(lsoas_sf)
unique(oxford_lsoas_sf$LSOA21NM)


# Use dplyr and stringr to filter rows containing 'oxford'
oxford_lsoas_sf <- lsoas_sf %>%
  filter(str_detect(LSOA21NM, regex("oxford|vale of white horse|cherwell", ignore_case = TRUE)))
...
plot <- ggplot(oxford_lsoas_sf)+ #plot a graph using cons_sf data (the entire dataset)
  geom_sf(fill = "#d946db", color = "grey88")+
    theme_minimal()
print(plot)

#make everything in the dataset filled in pink, with gray outlines
