...
```{r debug_paths, echo=TRUE}
# Debugging the resolved paths during knitting
print(here("data", "multdep_2019", "Indices_of_Multiple_Deprivation_(IMD)_2019", "Indices_of_Multiple_Deprivation_(IMD)_2019.shp"))
print(here("data", "postcode_boundary_data", "Distribution", "Sectors.shp"))

# Check if the files exist during knitting
print(file.exists(here("data", "multdep_2019", "Indices_of_Multiple_Deprivation_(IMD)_2019", "Indices_of_Multiple_Deprivation_(IMD)_2019.shp")))
print(file.exists(here("data", "postcode_boundary_data", "Distribution", "Sectors.shp")))
```
...
server <- function(input, output) {
  ...
  setwd(here())  # Reset working directory to the root of the project
  
  # Create a ggplot object (plot_experiment) using the data, example shown here:
  plot_experiment <- ggplot() +
    geom_sf(data = multdep) +  # Plot the multdep shapefile
    geom_sf(data = postcodes_sf, fill = "transparent", color = "blue")  # Plot the postcodes shapefile on top
  
  # Render the interactive Plotly map
...
ggplotly(plot_experiment)


```

```{r}
# Attempt a simple ggplot with your sf data
library(ggplot2)
library(sf)

multdep <- st_read(multdep_path)
postcodes_sf <- st_read(postcodes_sf_path)

# Try plotting the sf object directly
test <- ggplot() +
  geom_sf(data = multdep) + 
  ggtitle("Simple ggplot with sf data")
print(test)
